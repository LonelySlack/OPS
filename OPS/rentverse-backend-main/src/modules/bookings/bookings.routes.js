 ...
 *   post:
 *     summary: Sign a booking/lease agreement digitally (tenant or landlord)
 *     tags: [Bookings]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *               signature:
 *                 type: string
 *                 description: Base64 PNG data URL of the signature
 *     responses:
 *       200:
 ...
 *         description: Unauthorized
 *       403:
 *         description: Not authorized to sign this booking
 *       404:
 *         description: Booking not found
 */
router.post('/:id/sign', auth, async (req, res) => {
  ...
  try {
    ...
    // Permission check
    const isTenant = req.user.id === lease.tenantId
    const isLandlord = req.user.id === lease.landlordId

    if (!isTenant && !isLandlord) {
      return res.status(403).json({ success: false, message: 'You are not authorized to sign this agreement' })
    }
    ...
    // Update status
    const tenantSigned = isTenant ? true : !!lease.tenantSignature
    const landlordSigned = isLandlord ? true : !!lease.landlordSignature

    if (tenantSigned && landlordSigned) {
      data.signatureStatus = 'FULLY_SIGNED'
    } else if (tenantSigned) {
      data.signatureStatus = 'TENANT_SIGNED'
    } else if (landlordSigned) {
      data.signatureStatus = 'LANDLORD_SIGNED'
    } else {
